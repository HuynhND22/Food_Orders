...
import {Button, Card, Form, Input, Space, Table, Popconfirm, message, Modal, Select, Tooltip, Radio} from 'antd';
import { useRequest } from 'ahooks';
import {DeleteOutlined, EditOutlined, QuestionCircleOutlined, UndoOutlined} from '@ant-design/icons';
...
export default function Suppliers(props: {}) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: any, index: number) => {
        return <span>{status.map((value)=>{if(value.statusId == record.statusId) return value.name})}</span>;
      },
    },
    {
      title: 'Email',
    ...
    },
    {
      ...
      render: (text:string, record:any)=>{
        return ( record.ward && <span>{record.address + ' - '+  record.ward?.name + ' - '+ record.ward?.district?.name + ' - '+ record.ward?.district?.province?.name}</span>)
      }
    },
    {
        ...
        if (deleted == 'all') {
          return (
              <Space size='small'>
                <Tooltip title="Chỉnh sửa">
                  <Button
                      ...
                      onClick={() => {
                        ...
                        let form = {...record, provinceId: record.ward?.district?.province?.provinceId, districtId: record.ward?.district?.districtId}
                        if (form.provinceId) {
                ...
                <Tooltip title='Xóa'>
                  <Button type='primary' danger icon={<DeleteOutlined />} onClick={()=>handleRemove(record.supplierId)} />
                </Tooltip>
              </Space>
          );
  ...
  const checkNameUnique = debounce( async(cb:any, value: string, ignore?:string) => {
    try{
      await axiosClient.get(`/suppliers/check/unique?field=name&value=${value}&ignore=${ignore}`)
      cb(undefined)
    } catch(error) {
      cb(true)
    }
  }, 500)
  const checkEmailUnique = debounce( async(cb:any, value: string, ignore?:string) => {
    try{
      await axiosClient.get(`/suppliers/check/unique?field=email&value=${value}&ignore=${ignore}`)
      cb(undefined)
    } catch(error) {
      cb(true)
    }
  }, 500)
  const checkPhoneNumberUnique = debounce( async(cb:any, value: string, ignore?:string) => {
    try{
      await axiosClient.get(`/suppliers/check/unique?field=phoneNumber&value=${value}&ignore=${ignore}`)
      cb(undefined)
    } catch(error) {
      cb(true)
    }
  ...
  return (
    <div style={{ padding: 36 }}>
      <Card title='Thêm Nhà cung cấp mới' style={{ width: '100%' }}>
        <Form form={createForm} name='create-supplier' labelCol={{ span: 2 }} wrapperCol={{ span: 12 }}  onFinish={(value)=>onFinish(value)}>
          <Form.Item<FieldType>
          ...
          <Form.Item labelCol={{ span: 6 }} label="Address">
            <Space>
              <Form.Item
                  noStyle
              >
                <Select style={{width: '160px'}} placeholder="Tỉnh/Thành phố" onChange={(value)=>getDistricts(value)}>
                  {provinces.map((value:any)=>{
                    return <Option key={value.provinceId} value={value.provinceId}>{value.name}</Option>
                  })}
                </Select>
              </Form.Item>
              <Form.Item
                  noStyle
              >
                <Select style={{width: '170px'}} placeholder="Quận/Huyện" onChange={(value)=>getWards(value)}>
                  {districts.map((value:any)=>{
                    return <Option key={value.districtId} value={value.districtId}>{value.name}</Option>
                  })}
                </Select>
              </Form.Item>
              <Form.Item<FieldType>
              ...
              >
                <Select placeholder="Phường/Xã" style={{width: '180px'}}>
                  {wards.map((value:any)=>{
                    return <Option key={value.wardId} value={value.wardId}>{value.name}</Option>
                  })}
                </Select>
              </Form.Item>
              <Form.Item<FieldType>
                  ...
                  noStyle
              >
               <Input style={{width: '300px'}} placeholder='Địa chỉ cụ thể' />
              </Form.Item>
            </Space>
          </Form.Item>
      ...
      >
        <Form form={updateForm} name='update-supplier' labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}  onFinish={onUpdate}>
          <Form.Item<FieldType>
            label='Name' labelCol={{ span: 6 }}
            name='name'
            rules={[
          ...
          <Form.Item labelCol={{ span: 6 }} label="Địa chỉ">
            <Space>
              <Form.Item
                  ...
                  name={'provinceId'}
              >
                <Select style={{width: '150px'}} placeholder="Tỉnh/Thành phố" onChange={(value:number)=>getDistricts(value)}>
                  {provinces.map((value:any)=>{
                    return <Option key={value.provinceId} value={value.provinceId}>{value.name}</Option>
                  })}
                </Select>
              ...
              <Form.Item
                  ...
                  name={'districtId'}
              >
                <Select style={{width: '160px'}} placeholder="Quận/Huyện" onChange={(value)=>getWards(value)}>
                  {districts && districts.map((value:any)=>{
                    return <Option key={value.districtId} value={value.districtId}>{value.name}</Option>
                  })}
                </Select>
              </Form.Item>
            ...
            <Space style={{margin:'10px 0 0 0'}}>
              ...
              <Form.Item<FieldType>
                  ...
                  noStyle
              >
                <Input style={{width: '160px'}} placeholder='Địa chỉ cụ thể' />
              </Form.Item>
            </Space>
          </Form.Item>
          <Form.Item<FieldType> name='statusId' labelCol={{ span: 6 }} label='Trạng thái'  rules={[{ required: true}]} hasFeedback>
            <Select
              options={
              status.map((item: any) => {
          ...
          <Form.Item<FieldType>
            ...
            rules={[
              { required: true, message: 'Số điện thoại là bắt buộc!' },
              { pattern: /^\d{10,11}$/, message: 'Số điện thoại chưa chính xác!' },
              {
                validator(rule, value, callback) {
                  checkPhoneNumberUnique(callback, value, selectedSupplier.phoneNumber);
                }, message: 'Số điện thoại đã tồn tại!'
              },
            ]}
            hasFeedback
